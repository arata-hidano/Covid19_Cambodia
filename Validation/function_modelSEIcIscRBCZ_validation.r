                                    ...
                                    )
{
  ...
  # Impact of use of floor, round, and ceiling - note in this code baseline is ceiling
  
  # Create dummy state variables for floor
  FA_f = BED_f = ICU_f = array(0,c(numSteps,length(pop$p_age)))
  cum_FA_f = cum_BED_f = cum_ICU_f = array(0,c(numSteps,length(pop$p_age)))
  FA_f[1,] = 0
  BED_f[1,] = 0
  ICU_f[1,] = 0
  cum_FA_f[1,] = cum_BED_f[1,] = cum_ICU_f[1,] = 0
  
  # Create dummy state variables for Round
  FA_r = BED_r = ICU_r = array(0,c(numSteps,length(pop$p_age)))
  cum_FA_r = cum_BED_r = cum_ICU_r = array(0,c(numSteps,length(pop$p_age)))
  FA_r[1,] = 0
  BED_r[1,] = 0
  ICU_r[1,] = 0
  cum_FA_r[1,] = cum_BED_r[1,] = cum_ICU_r[1,] = 0
  ...
  { 
    ...
    # Evaluate the impacy of using ceiling/floor
    numEtoF_f   = rbinom(length(numEtoIc), floor(numEtoIc), delta)    # E to FA
    numFtoB_f = rbinom(length(FA_f[stepIndex,]), FA_f[stepIndex,], theta*(1-epsilon)*dt) # this only valid if time step is a day, otherwise use Poisson
    numFtoC_f = rbinom(length(FA_f[stepIndex,]), FA_f[stepIndex,], theta*epsilon*dt)
    numBtoZ_f = rbinom(length(BED_f[stepIndex,]),BED_f[stepIndex,],zeta_B)
    numCtoZ_f = rbinom(length(ICU_f[stepIndex,]),ICU_f[stepIndex,],zeta_C)
    
    # calculate cumulative number of new Ic, FA, BED, ICU
    cum_FA_f[stepIndex+1,] = cum_FA_f[stepIndex,] + numEtoF_f
    cum_BED_f[stepIndex+1,] = cum_BED_f[stepIndex,] + numFtoB_f
    cum_ICU_f[stepIndex+1,] = cum_ICU_f[stepIndex,] + numFtoC_f
    
    # Evaluate the impacy of using roundup
    numEtoF_r   = rbinom(length(numEtoIc), round(numEtoIc), delta)    # E to FA
    numFtoB_r = rbinom(length(FA_r[stepIndex,]), FA_r[stepIndex,], theta*(1-epsilon)*dt) # this only valid if time step is a day, otherwise use Poisson
    numFtoC_r = rbinom(length(FA_r[stepIndex,]), FA_r[stepIndex,], theta*epsilon*dt)
    numBtoZ_r = rbinom(length(BED_r[stepIndex,]),BED_r[stepIndex,],zeta_B)
    numCtoZ_r = rbinom(length(ICU_r[stepIndex,]),ICU_r[stepIndex,],zeta_C)
    
    # calculate cumulative number of new Ic, FA, BED, ICU
    cum_FA_r[stepIndex+1,] = cum_FA_r[stepIndex,] + numEtoF_r
    cum_BED_r[stepIndex+1,] = cum_BED_r[stepIndex,] + numFtoB_r
    cum_ICU_r[stepIndex+1,] = cum_ICU_r[stepIndex,] + numFtoC_r
    ...
    # For floor and roundup
    FA_f[stepIndex+1,]   = FA_f[stepIndex,]+numEtoF_f-numFtoB_f-numFtoC_f;
    FA_f[stepIndex+1,] <-ifelse(FA_f[stepIndex+1,]<0,0,FA_f[stepIndex+1,])
    
    BED_f[stepIndex+1,]   =BED_f[stepIndex,]+numFtoB_f-numBtoZ_f;
    ICU_f[stepIndex+1,]   = ICU_f[stepIndex,]+numFtoC_f-numCtoZ_f;
    
    FA_r[stepIndex+1,]   = FA_r[stepIndex,]+numEtoF_r-numFtoB_r-numFtoC_r;
    FA_r[stepIndex+1,] <-ifelse(FA_r[stepIndex+1,]<0,0,FA_r[stepIndex+1,])
    
    BED_r[stepIndex+1,]   =BED_r[stepIndex,]+numFtoB_r-numBtoZ_r;
    ICU_r[stepIndex+1,]   = ICU_r[stepIndex,]+numFtoC_r-numCtoZ_r;
    
  }
  output = list(S = S, E = E, Ic = Ic, Isc = Isc, R = R, BED= BED, ICU=ICU,Z=Z, time = time, lambda=lambda,
                BED_det= BED_det, ICU_det=ICU_det,
                BED_f= BED_f, ICU_f=ICU_f,
                cum_Ic = cum_Ic, cum_FA = cum_FA, cum_BED = cum_BED, cum_ICU = cum_ICU,
                cum_FA_f = cum_FA_f, cum_BED_f = cum_BED_f, cum_ICU_f = cum_ICU_f,
                cum_FA_r = cum_FA_r, cum_BED_r = cum_BED_r, cum_ICU_r = cum_ICU_r,
